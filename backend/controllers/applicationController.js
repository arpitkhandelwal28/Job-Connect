...
export const postApplication = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    const cloudinaryResponse = await cloudinary.uploader.upload(
      resume.tempFilePath,
       { resource_type: "raw", flags: "inline", public_id: `resumes/${Date.now()}_${resume.name}`,use_filename: true,unique_filename: false }
    );
...
export const jobseekerDeleteApplication = catchAsyncErrors(
  async (req, res, next) => {
    ...
    try {
      ...
      return next(new ErrorHandler("Failed to delete application properly", 500));
    }
  }
);
